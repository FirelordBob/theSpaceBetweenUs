...
if(instance_exists(player) && distance_to_object(player) < range && !firing)
{
	...
	if(ready && vision && currentHealth > 0)
	{
		...
		if(attackMode == 0){
			...
			instance_create_depth(x + lengthdir_x(sprite_width/2,angle),y + lengthdir_y(sprite_width/2,angle),depth-1,obj_spacemine)
			instance_create_depth(x + lengthdir_x(sprite_width/2,angle) + 50 ,y + lengthdir_y(sprite_width/2,angle) + irandom_range(-30,30),depth-1,obj_spacemine)
			instance_create_depth(x + lengthdir_x(sprite_width/2,angle) - 50 ,y + lengthdir_y(sprite_width/2,angle) + irandom_range(-30,30),depth-1,obj_spacemine)
			audio_play_sound(snd_enemylazersuper,51,0);
			attackMode = 2;
		} else {
			instance_create_depth(x + lengthdir_x(sprite_width/2,angle)+50,y + lengthdir_y(sprite_width/2,angle),depth-1,obj_asteroid)
			instance_create_depth(x + lengthdir_x(sprite_width/2,angle)-50,y + lengthdir_y(sprite_width/2,angle) + irandom_range(-30,30),depth-1,obj_asteroid)
			audio_play_sound(snd_enemylazersuper,51,0);
			attackMode = 0;
		}
